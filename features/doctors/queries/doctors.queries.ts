...
export async function getDoctors(): Promise<Doctor[]> {
    ...
    SELECT id, name_en, name_ar, names, specialty_en, specialty_ar, specialties,
           bio_en, bio_ar, bios, image_url, experience_years, is_active, created_at
    FROM doctors
...
export async function getDoctorsBySubService(subServiceId: string): Promise<Doctor[]> {
    const rows = await sql`
    SELECT d.id, d.name_en, d.name_ar, d.names, d.specialty_en, d.specialty_ar, d.specialties,
           d.bio_en, d.bio_ar, d.bios, d.image_url, d.experience_years, d.is_active,
